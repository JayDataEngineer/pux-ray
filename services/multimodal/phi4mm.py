...
class Phi4MMService(ForgeService):
    """Forge adapter for Phi-4-multimodal (~24GB VRAM).

    5.6B parameter omni model supporting text, vision, audio, and vision+audio modes.
    """

    vram_mb = 24_576
    service_name = "phi4mm"
    default_model = "phi4-multimodal"
    ...
    def infer(self, payload: dict) -> dict:
        ...
        if mode == "text":
            ...
            prompt = self._format_chat(messages)
            content = self._run_text_inference(prompt, max_tokens, 0.7)
        elif mode == "vision":
            image_input = payload.get("image")
            if not image_input:
                return {"status": "error", "error": "image (bytes or base64) required for vision mode"}
            image_bytes = _b64_decode(image_input) if isinstance(image_input, str) else image_input
            image = Image.open(io.BytesIO(image_bytes)).convert("RGB")
            content = self._run_vision_inference(image, prompt_text or "Describe the image in detail.", max_tokens)
        elif mode == "audio":
            audio_input = payload.get("audio")
            if not audio_input:
                return {"status": "error", "error": "audio (bytes or base64) required for audio mode"}
            audio_bytes = _b64_decode(audio_input) if isinstance(audio_input, str) else audio_input
            import soundfile as sf
            audio, sr = sf.read(io.BytesIO(audio_bytes))
            content = self._run_audio_inference(audio, sr, prompt_text or "Transcribe the audio clip into text.", max_tokens)
        elif mode == "vision_audio":
            image_input = payload.get("image")
            audio_input = payload.get("audio")
            if not image_input or not audio_input:
                return {"status": "error", "error": "image and audio both required for vision_audio mode"}
            image_bytes = _b64_decode(image_input) if isinstance(image_input, str) else image_input
            audio_bytes = _b64_decode(audio_input) if isinstance(audio_input, str) else audio_input
            image = Image.open(io.BytesIO(image_bytes)).convert("RGB")
            import soundfile as sf
            audio, sr = sf.read(io.BytesIO(audio_bytes))
            content = self._run_vision_audio_inference(image, audio, sr, prompt_text, max_tokens)
        else:
            return {"status": "error", "error": f"Unknown mode: {mode}"}

        return {"status": "ok", "content": content, "mode": mode}
