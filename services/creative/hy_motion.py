...
class HYMotionService(ForgeService):
    """Forge-managed HY-Motion text-to-3D human motion service."""
    vram_mb = 6_144
    ...
    def load(self, model_name: str) -> None:
        ...
        # Simple config object for precision tracking (no BaseGPUDeployment dependency)
        class _Cfg:
            precision = "bf16"
            low_resource = False
            def dtype(self):
                import torch
                return {"fp32": torch.float32, "fp16": torch.float16}.get(self.precision, torch.bfloat16)
