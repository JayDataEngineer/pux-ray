"""ACE-STEP — Music generation from text prompts.

Generates music from text descriptions. Runs as a subprocess within
the KubeRay worker pod.
Requires ~8GB VRAM.
"""
from __future__ import annotations

import logging

from ray import serve

from services.base import BaseGPUDeployment, SubprocessProxyMixin

logger = logging.getLogger(__name__)


@serve.deployment(
    name="ace_step",
    num_replicas=1,
    max_ongoing_requests=1,
    ray_actor_options={"num_gpus": 0, "num_cpus": 0.5},
)
class ACEStepDeployment(BaseGPUDeployment, SubprocessProxyMixin):
    """ACE-STEP music generation via subprocess proxy."""

    SUBPROCESS_PORT = 9000

    def _load(self, model_name: str = "ace-step") -> None:
        self._start_proxy(
            cmd=["python", "-m", "uvicorn", "api:app",
                 "--host", "0.0.0.0", "--port", str(self.SUBPROCESS_PORT)],
            port=self.SUBPROCESS_PORT,
            health_path="/health",
            timeout=300,
            cwd="/opt",
            env={
                "ACESTEP_CHECKPOINTS_DIR": "/models/audio/acestep",
                "HF_HOME": "/root/.cache/huggingface",
                "PYTHONPATH": "/app:/opt/acestep:/opt",
            },
        )
        self.model = True
        self.model_name = model_name

    def _unload(self) -> None:
        self._stop_proxy()
        self.model = None

    async def __call__(self, request):
        await self._ensure_loaded("ace-step")
        return await self._proxy_request(request)
