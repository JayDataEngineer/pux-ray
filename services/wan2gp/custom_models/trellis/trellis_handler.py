...
Decomposes the TRELLIS pipeline into individual nn.Modules for mmgp VRAM management.
Own inference logic: preprocess → conditioning → SS sampling → SLAT cascade → decode → GLB.
...
class family_handler:
    ...
    def load_model(model_filename, model_type, base_model_type, model_def,
        ...
        # Map: short_name → pipeline.models key
        key_map = {
        ...
        # Image conditioning (DINOv2) — stored as pipeline attribute
        if hasattr(pipeline, 'image_cond_model') and pipeline.image_cond_model is not None:
            cond = pipeline.image_cond_model
            if isinstance(cond, torch.nn.Module):
                cond = cond.to(dev)
            elif hasattr(cond, 'to'):
                cond.to(dev)
            pipe['image_cond'] = cond

        # pipeline.to(dev) doesn't move pipeline.models dict entries
...
class _Pipeline:
    ...
    def generate(self, *, image=None, seed=1, steps=12, guidance=7.5,
        ...
        # 1. Preprocess image (background removal)
        img = self._preprocess_image(img)

        # 2. Image conditioning (DINOv2/v3 features)
        ...
        # 4. Shape SLAT sampling (cascade or single)
        if resolution == '512':
    ...
    def _preprocess_image(self, img):
        ...
        if max_size > 1024:
            scale = 1024 / max_size
            img = img.resize((int(img.width * scale), int(img.height * scale)),
                             __import__("PIL").Image.Resampling.LANCZOS)
        ...
        if has_alpha:
            ...
            if len(bbox) > 0:
                ...
                bg = __import__("PIL").Image.new("RGB", img.size, (255, 255, 255))
    ...
    def _decode_latent(self, shape_slat, tex_slat, resolution):
        ...
        # Offload tex_slat during shape decode
        tex_slat = tex_slat.to('cpu')
        torch.cuda.empty_cache()

        # Shape decode — FDG VAE decoder returns (meshes, subs) in inference
        shape_dec = self.m['shape_decoder']
