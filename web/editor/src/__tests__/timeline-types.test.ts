...
describe('Timeline Types — constants', () => {
  ...
  it('DEFAULT_SEGMENT_PARAMS has reasonable defaults', () => {
    ...
    expect(DEFAULT_SEGMENT_PARAMS.samplingSteps).toBeGreaterThan(0)
    expect(DEFAULT_SEGMENT_PARAMS.model).toBeTruthy()
    // Wan 1.3B defaults
    expect(DEFAULT_SEGMENT_PARAMS.width).toBe(1280)
    expect(DEFAULT_SEGMENT_PARAMS.height).toBe(720)
    expect(DEFAULT_SEGMENT_PARAMS.fps).toBe(16)
    expect(DEFAULT_SEGMENT_PARAMS.frames).toBe(81)
    expect(DEFAULT_SEGMENT_PARAMS.model).toBe('wan/t2v_1.3B')
  })
})
